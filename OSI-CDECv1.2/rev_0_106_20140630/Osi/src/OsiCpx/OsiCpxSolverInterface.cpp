...
void OsiCpxSolverInterface::initialSolve()
{
  ...
  getHintParam(OsiDoBarrierInInitial,takeHint,strength);
  if (strength!=OsiHintIgnore)
     algorithm = takeHint ? -2 : algorithm;

  getHintParam(OsiDoNetoptInInitial,takeHint,strength);
  if (strength!=OsiHintIgnore)
     algorithm = takeHint ? -3 : algorithm;
  ...
  switch( algorithm ) {
     ...
     case -1:
        ...
        break;
     case -2:
        term = CPXhybbaropt (env_, lp, 0);
        checkCPXerror( term, "CPXhybbaropt", "initialSolve" );
        break;	
     case -3:
        term = CPXhybnetopt (env_, lp, CPX_ALG_DUAL);
        checkCPXerror( term, "CPXhybnetopt", "initialSolve" );
...
void OsiCpxSolverInterface::resolve()
{
  ...
  getHintParam(OsiDoBarrierInResolve,takeHint,strength);
  if (strength!=OsiHintIgnore)
     algorithm = takeHint ? -2 : algorithm;

  getHintParam(OsiDoNetoptInResolve,takeHint,strength);
  if (strength!=OsiHintIgnore)
     algorithm = takeHint ? -3 : algorithm;
...
std::string 
OsiCpxSolverInterface::solverVersion() const
{
  debugMessage("OsiCpxSolverInterface::solverVersion()\n");
  if (env_) 
    {
      std::string version("CPLEX ");
      return version + CPXversion( env_ );
    }
  else
    {
      return "Unknown";
...
double 
OsiCpxSolverInterface::getConditionNumber() const
{
  debugMessage("OsiCpxSolverInterface::getConditionNumber()\n");

  double retval = -1;

  int err = CPXgetdblquality(env_, getMutableLpPtr(), &retval, CPX_KAPPA);
  checkCPXerror( err, "CPXgetdblquality", "getConditionNumber" );
...
OsiCpxSolverInterface::setIntParam(OsiIntParam key, int value)
{
  ...
  switch (key)
    ...
    case OsiNameDiscipline:
       ...
       break;
    case OsiNumThreads:
       retval = ( CPXsetintparam( env_, CPX_PARAM_THREADS, value ) == 0 );
       break;
    case OsiParallelMode:
       retval = ( CPXsetintparam( env_, CPX_PARAM_PARALLELMODE, value ) == 0 );
       break;
    case OsiMIPSubAlgo:
    {
       int val = 0;
	 ...
	 val = CPX_ALG_AUTOMATIC;
       ...
       retval = ( CPXsetintparam( env_, CPX_PARAM_STARTALG, val ) == 0 )
	     && ( CPXsetintparam( env_, CPX_PARAM_SUBALG, val ) == 0 );
       break;
    }
...
OsiCpxSolverInterface::setDblParam(OsiDblParam key, double value)
{
  ...
  switch (key)
    ...
    case OsiPrimalTolerance:
      ...
      break;
    case OsiBarrierTolerance:
      retval = ( CPXsetdblparam( env_, CPX_PARAM_BAREPCOMP, value ) == 0 ); 
      break;
    ...
    case OsiObjOffset:
      ...
      break;
    case OsiObjRange:
      retval =( CPXsetdblparam( env_, CPX_PARAM_BAROBJRNG, value) == 0);
      break;
    case OsiTimeLimit:
      retval = ( CPXsetdblparam( env_, CPX_PARAM_TILIM, value) == 0 ); 
      // retval = ( CPXsetdblparam( env_, CPX_PARAM_POLISHAFTERTIME, value) == 0);
      break;
    case OsiMIPGap:
      retval = ( CPXsetdblparam( env_, CPX_PARAM_EPGAP, value ) == 0 ); 
      // retval = ( CPXsetdblparam( env_, CPX_PARAM_POLISHAFTEREPGAP, value) == 0);
      break;
...
double OsiCpxSolverInterface::getCoefficient(int rowIndex, int colIndex) const
{
  debugMessage("OsiCpxSolverInterface::getCoefficient(%d, %d)\n", rowIndex, colIndex);

  double coef;
  int err = CPXgetcoef(env_, getMutableLpPtr(), rowIndex, colIndex, &coef);
  checkCPXerror( err, "CPXgetobj", "getCoefficient" );
  return coef;
}

/* Set a coefficient of a column in a row */
void
OsiCpxSolverInterface::setCoefficient(int rowIndex, int colIndex, double elementValue)
{
  debugMessage("OsiCpxSolverInterface::setCoefficient(%d, %d, %f)\n", rowIndex, colIndex, elementValue);
  // int err = CPXchgcoef(env_, getLpPtr( OsiCpxSolverInterface::FREECACHED_MATRIX ), rowIndex, colIndex, elementValue);
  int err = CPXchgcoef(env_, getLpPtr( ), rowIndex, colIndex, elementValue);

  checkCPXerror( err, "CPXchgcoef", "setCoefficient" );
}
...
int 
OsiCpxSolverInterface::getExtremeRay(double *ray, double * rhs)
{
  int err = CPXdualfarkas(env_, getMutableLpPtr(), ray, rhs);
  checkCPXerror(err, "CPXdualfarkas", "getExtremeRays");
...
void OsiCpxSolverInterface::writeLps(const char *filename,
				     const char *extension) const
{
  debugMessage("OsiCpxSolverInterface::writeLps(%s, %s)\n", filename, extension);
  ...
  { 
    ...
    std::vector<const char*> newnames(cnt);
    std::vector<std::string> names(cnt);
    
    for (int i = 0; i < cnt; ++i) {
      names[i] = getColName(i);
      indices[i] = i;
      newnames[i] = names[i].c_str();
    }
    
    int err = CPXchgcolname (env_, getMutableLpPtr(), cnt,
			     &(indices[0]), const_cast<char **>(&(newnames[0])));

    checkCPXerror( err, "CPXchgcolname", "writeLps" );
  }
  ...
  { 
    ...
    int err = CPXchgrowname (env_, getMutableLpPtr(), cnt,
			     &(indices[0]), const_cast<char **>(&(newnames[0])));

    checkCPXerror( err, "CPXchgrowname", "writeLps" );
  }
  
  {
    std::string name;
    getStrParam(OsiProbName, name);
    int err = CPXchgprobname (env_, getMutableLpPtr(), name.c_str());

    checkCPXerror( err, "CPXchgprobname", "writeLps" );
  }


  int err = CPXlpwrite( env_, getMutableLpPtr(), const_cast<char*>( fullname.c_str() ));
  checkCPXerror( err, "CPXlpwrite", "writeLps" );
...
OsiSolverInterface * OsiCpxSolverInterface::clone(bool copyData) const
{
  ...
  return( new OsiCpxSolverInterface( *this, copyData ) );
}
...
bool 
OsiCpxSolverInterface::setHintParam(OsiHintParam key, bool yesNo,
                                    OsiHintStrength strength,
