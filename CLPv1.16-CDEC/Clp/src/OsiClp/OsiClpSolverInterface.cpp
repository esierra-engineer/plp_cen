...
#endif
/***************************************************************************/
//AAMS PLP Additional public methods
double OsiClpSolverInterface::getCoefficient(int irow, int icol) const
{
	int nrows = getNumRows();
	int ncols = getNumCols();
	if (irow < 0 || irow >= nrows)
	{
		std::cerr<<"Illegal row index "<<irow<<" in OsiClpSolverInterface::"<<"getCoefficient"<<std::endl;
 		throw CoinError("Illegal row index","getCoefficient","OsiClpSolverInterface");
	}	
	if (icol < 0 || icol >= ncols )
	{		
		std::cerr<<"Illegal column index "<<icol<<" in OsiClpSolverInterface::"<<"getCoefficient"<<std::endl;
 		throw CoinError("Illegal column index","getCoefficient","OsiClpSolverInterface");
	}
	const CoinPackedMatrix * matrix = getMatrixByCol();
	
	
	int start = matrix->getVectorStarts()[icol]; 
	int end = start + matrix->getVectorLengths()[icol];
	for( int j = start; j < end; j++)
	{
		int row = matrix->getIndices()[j];
		if (row == irow)
			return matrix->getElements()[j];
		if (row > irow)
			return 0.0;
	}
	return 0.0;
...
void OsiClpSolverInterface::setCoefficient(int irow, int icol, double value) 
{
	int nrows = getNumRows();
	int ncols = getNumCols();
	...
	if (irow < 0 || irow >= nrows)
	{
		std::cerr<<"Illegal row index "<<irow<<" in OsiClpSolverInterface::"<<"setCoefficient"<<std::endl;
 		throw CoinError("Illegal row index","setCoefficient","OsiClpSolverInterface");
	}	
	if (icol < 0 || icol >= ncols )
	{		
		std::cerr<<"Illegal column index "<<icol<<" in OsiClpSolverInterface::"<<"setCoefficient"<<std::endl;
 		throw CoinError("Illegal column index","setCoefficient","OsiClpSolverInterface");
	}
	
	modifyCoefficient(irow, icol, value);	
}

std::string OsiClpSolverInterface::solverVersion() const
{
	std::string solver("CLP ");
        std::string m("m");
	return solver + CLP_VERSION + m;
}
  
double OsiClpSolverInterface::getConditionNumber() const
{
	return 1.0;
}
  
int
OsiClpSolverInterface::getExtremeRay(double *ray, double * rhs)
{
	// We are following CPLEX explanation for dualfarkas routine
	// Defining local tolerance to consider a value as zero to avoid numerical issues
	const double TOL = 1e-9;
	// Defining local tolerance to eliminate too large values (COIN_DBL_MAX, for instance)
	const double MTOL = 1e50;
	// modelPtr_ is an ClpSimplex instance
	// clp returns a recession direction NOT necessarily an extreme ray
	// this procedure returns a farkas certificate NOT necessarily an extreme ray
	ClpSimplex * model = getModelPtr();
	if (!isProvenPrimalInfeasible()) return 1;

	// We need to unscale the matrix if scaling took place
        ClpPackedMatrix * mscale = model->swapScaledMatrix(NULL);
        double * rscale = model->swapRowScale(NULL);

	double * clpray = model->infeasibilityRay();
	
	if (clpray == NULL) return 1;
	if (ray == NULL) return 1;
	
	// current problem data (clp)
	int nrows = getNumRows();
	int ncols = getNumCols();
	const double * lb = getColLower();
	const double * ub = getColUpper();
	// we recover lower an upper RHSs to avoid non definite bounds
	const double * lrhs = getRowLower();
	const double * urhs = getRowUpper();
	
	// local arrays zero initialized
	double * b = new double[nrows];
	double * yA = new double[ncols];
	double * z = new double[ncols];
	memset(b, 0.0, nrows * sizeof(double));
	memset(yA, 0.0, ncols * sizeof(double));
	memset(z, 0.0, ncols * sizeof(double));
	
	// local vars
	double arhs = 0.0;
	
	// we eliminate near zero values of clpray and non definite bounds for variables (FREE variables, for instance)
	double norm = 1.0;
        double mray = 1e50;
        double Mray = 0.0;
	for (int irow = 0; irow < nrows; irow++)
	{
		if (clpray[irow] > TOL)
		{
			if (fabs(urhs[irow]) < MTOL)
			{
				b[irow] = urhs[irow];
				ray[irow] = -clpray[irow];
                                if (fabs(ray[irow]) > Mray)
                                        Mray = fabs(ray[irow]);
				if (fabs(ray[irow]) < mray && fabs(ray[irow]) != 0.0)
					mray = fabs(ray[irow]);
				if (norm == 1.0 && fabs(ray[irow]) > 1)
					norm = ray[irow];
				continue;
			}
		}
	...
	// computing y'*A
	model->transposeTimes(-1.0, clpray, yA);
	
	// setting bounds on z according to respective y'*A sign
	for (int icol = 0; icol < ncols; icol++)
	{
		if (yA[icol] > TOL)
		{
			if (fabs(ub[icol]) < MTOL)
				z[icol] = ub[icol];
			continue;
		}
		if (yA[icol] < -TOL)
		{
			if (fabs(lb[icol]) < MTOL)
				z[icol] = lb[icol];
			continue;
		}
	}
	
	// computing y'*b - y'*A*z, farkas proof
	model->times(-1.0, z, b);

        // if difference is too high, disable final scaling
        if (Mray/fabs(norm) > 1e4)
		norm = 1.0;
	// uncomment next line in order to disable final scaling
	//norm = 1.0;	 
	for (int irow = 0; irow < nrows; irow++)
	{
		ray[irow] = ray[irow] / fabs(norm);
		arhs += ray[irow] * b[irow];
	}
	
	*rhs = arhs;

        // restoring scaling
	model->swapRowScale(rscale);
        model->swapScaledMatrix(mscale);
