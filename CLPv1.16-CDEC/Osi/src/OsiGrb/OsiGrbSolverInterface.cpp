...
void OsiGrbSolverInterface::initialSolve()
{
  ...
  GUROBI_CALL( "initialSolve", GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_OUTPUTFLAG, (messageHandler()->logLevel() > 0)) );
       ...
       /* save infeasible info for retreival */
  GUROBI_CALL( "initialSolve", GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_INFUNBDINFO, 1));
...
void OsiGrbSolverInterface::resolve()
{
  ...
  GUROBI_CALL( "resolve", GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_OUTPUTFLAG, (messageHandler()->logLevel() > 0)) );
       ...
       /* save infeasible info for retreival */
  GUROBI_CALL( "resolve", GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_INFUNBDINFO, 1));
...
std::string 
OsiGrbSolverInterface::solverVersion() const
{
  debugMessage("OsiGrbSolverInterface::solverVersion()\n");

  std::ostringstream version;
  
  version << "Gurobi " << GRB_VERSION_MAJOR << "." << GRB_VERSION_MINOR << "." << GRB_VERSION_TECHNICAL;
...
double 
OsiGrbSolverInterface::getConditionNumber() const
{
  debugMessage("OsiGrbSolverInterface::getConditionNumber()\n");

  double retval = -1;

  GUROBI_CALL( "getConditionNumber", GRBgetdblattr(getMutableLpPtr(), GRB_DBL_ATTR_KAPPA_EXACT, &retval) );
...
//  	  break;
  ...
  case OsiObjRange:
    return OsiSolverInterface::setDblParam(key,value);
  case OsiTimeLimit:
    if( GRBsetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_TIMELIMIT, value) != 0 )
    {
      *messageHandler() << "Warning: Setting GUROBI time limit" << value << "failed." << CoinMessageEol;
      if (OsiGrbSolverInterface::globalenv_)
	*messageHandler() << "\t GUROBI error message: " << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
      return false;
    }
    return true;
  case OsiMIPGap:
    if( GRBsetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_MIPGAP, value) != 0 )
    {
      *messageHandler() << "Warning: Setting GUROBI MIP gap" << value << "failed." << CoinMessageEol;
      if (OsiGrbSolverInterface::globalenv_)
	*messageHandler() << "\t GUROBI error message: " << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
      return false;
    }
...
double OsiGrbSolverInterface::getCoefficient(int rowIndex, int colIndex) const
{
  debugMessage("OsiGrbSolverInterface::getCoefficient(%d, %d)\n", rowIndex, colIndex);

  double coef;
  GUROBI_CALL( "getCoefficient", GRBgetcoeff(getMutableLpPtr(), rowIndex, colIndex, &coef) );
...
void
OsiGrbSolverInterface::setCoefficient(int rowIndex, int colIndex, double elementValue)
{
  debugMessage("OsiGrbSolverInterface::setCoefficient(%d, %d, %f)\n", rowIndex, colIndex, elementValue);
  
  GUROBI_CALL( "setCoefficient", GRBchgcoeffs(getLpPtr( ), 1, &rowIndex, &colIndex, &elementValue));
}
...
int 
OsiGrbSolverInterface::getExtremeRay(double *ray, double * rhs)
{
  int n = getNumRows();

  debugMessage("OsiGrbSolverInterface::getExtremeRay()\n");

  GUROBI_CALL( "getExtremeRay", GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_FARKASDUAL, 0, n, ray) );
  ...
  GUROBI_CALL( "getExtremeRay", GRBgetdblattr(getMutableLpPtr(), GRB_DBL_ATTR_FARKASPROOF, rhs) );
