...
std::string 
OsiSolverInterface::solverVersion() const
{
  return "Unknown";
}

double 
OsiSolverInterface::getConditionNumber() const
{
  return -1;
}
...
void OsiSolverInterface::writeLps(const char *filename,
				  const char *extension) const
{						
  double eps = 1.0e-12;
  int numberAcross = 8;
  int decimals = 15;
  writeLp(filename, extension, eps, numberAcross, decimals);
}
				  ...
				  const bool useRowNames) const
{
   ...
   std::string pname;
   if (getStrParam(OsiProbName, pname))
     {
       writer.setProblemName(pname.c_str());
     }
...
#include <stdexcept>
/* Get a coefficient of a column in a row */
double 
OsiSolverInterface::getCoefficient(int , int ) const
{
...
int 
OsiSolverInterface::getExtremeRay(double *, double *)
{
  throw std::runtime_error("not implemented");
}
...
void
OsiSolverInterface::setCoefficient(int , int , double )
{
  throw std::runtime_error("not implemented");
}
